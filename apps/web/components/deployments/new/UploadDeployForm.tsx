        ...
        <div className="rounded-md border border-[#1F1F1F] bg-[#0A0A0A] p-6 space-y-6 font-mono text-xs">
            ...
            {mode === 'production' ? (
                <HostingConfiguration
                    ...
                    ipPreview={selectedVps ? `${(projectName || (file ? file.name.replace(/\.(zip|tar\.gz|tgz)$/i, '') : 'app')).toLowerCase().replace(/[^a-z0-9-]/g, '-')}-preview.${selectedVps.ipAddress}.sslip.io` : 'Select a VPS node to preview ingress URL'}
                    error={errors.domain}
                />
            ) : (
                <div className="rounded-md border border-[#1F1F1F] bg-[#111111] p-4 text-xs text-[#A1A1A1]">
