...
export default function GoogleCallbackPage() {
    return (
        <Suspense fallback={<GoogleCallbackFrame><GoogleCallbackShell /></GoogleCallbackFrame>}>
...
function GoogleCallbackContent() {
    ...
    return (
        <GoogleCallbackFrame>
            <GoogleCallbackShell error={error} />
        </GoogleCallbackFrame>
    );
}

function GoogleCallbackFrame({ children }: { children: ReactNode }) {
    return (
...
function GoogleCallbackShell({ error }: { error?: string | null }) {
    return (
        <section className="w-full max-w-md rounded-lg border border-white/10 bg-slate-900/75 p-6 text-center shadow-2xl shadow-slate-950/40 backdrop-blur-xl">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-lg border border-cyan-300/20 bg-cyan-300/10 text-cyan-100">
                {error ? <Chrome size={24} /> : <Loader2 className="animate-spin" size={24} />}
            </div>
            <h1 className="mt-6 text-2xl font-black tracking-tight text-white">{error ? 'Google login failed' : 'Completing Google login'}</h1>
